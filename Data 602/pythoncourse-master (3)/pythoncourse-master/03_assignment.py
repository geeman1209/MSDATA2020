...
class MongoDB():
    def __init__(self):
        self.myhash = collections.defaultdict(dict)
        self.myhash['default']['version'] = 1.0
        self.myhash['default']['db'] = 'mongodb'
        self.myhash['default']['uuid'] = str(uuid.uuid4())
        
    def print_collections(self):
        print(json.dumps(self.myhash,sort_keys=True, indent=4))
        
    def add_collection(self, name):
        self.myhash[name] = dict()
        
    def update_collection(self, name, my_dict):
        self.myhash[name] = {**self.myhash[name], **my_dict}
        
    def remove_collections(self, name):
        return self.myhash.pop(name, None)
    ...
    def get_collection_names(self):
        return list(self.myhash.keys())
        
    def get_collection_size(self, name):
        if name in self.myhash.keys():
            return len(self.myhash[name])
        else:
            return
        
    def to_json(self):
        return json.dumps(self.myhash,sort_keys=True, indent=4)
...
def exercise01():
    ...
    '''
        ...
        instantiation via the constructor). Make sure to use Python 3 semantics. Make sure the length and width attributes are private and accessible only via getters. 
...
def exercise02():
    ...
    my_list = [12,343,3425,132,342,2344,6,1,3,543]
    db.update_collection('testscores', {i+1:my_list[i] for i in range(len(my_list))})
    db.get_collection_size('testscores')
    db.list_collections()
    print(db.myhash['default']['uuid'])
    db.wipe()
    print(db.myhash['default']['uuid'])
    # ------ Place code above here /\ /\ /\ ------
