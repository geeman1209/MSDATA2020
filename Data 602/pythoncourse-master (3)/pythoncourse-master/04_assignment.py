...
class PandasChain:
    ...
    def __init__(self, name): 
        ...
        self.__current_block = Block(self.__seqid, self.__prev_hash) # Create a new Block 
    ...
    def __commit_block(self,block):
        ...
        hashstring = str(self.__prev_hash) + str(self.__id) + str(dt.datetime.now()) + str(self.__seqid) + block.get_simple_merkle_root()
        block_hash.update(hashstring.encode('utf-8'))
        
        self.__block_hash = block_hash.hexdigest()
        block.set_block_hash(self.__block_hash)
        self.__prev_hash = self.__block_hash
        self.__chain.append(self.__curent_block)
        self.__seqid = self.__seqid + 1
        self.__curent_block = Block(self.__seqid, self.__prev_hash)
        # Add code here
    ...
    def display_block_headers(self): 
        ...
        self.__curent_block.display_header()
    ...
    def get_values(self):
        ...
        for block in self.__chain:
            blockValues, timeStamps = block.get_values()
            for i in blockValues:
                coins.append(i)
            for j in timeStamps:
                transacts.append(j)
                
        # Get the current block
        currentBlockValues, currentTimeStampValues = self.__curent_block.get_values()
        for i in currentBlockValues:
            coins.append(i)
            
        for j in currentTimeStampValues:
            transacts.append(j)
        
        return([transacts, coins])
...
class Block:
    ...
    def display_header(self): 
        print('seqid : {} Block status: {} Prev Hash: {} merkleroot: {} block size: {}'.format(self.__seq_id, self.__status, self.__block_hash, self.__prev_hash, self.__merkle_tx_hash, self.__transactions.shape[1][-1]))
    ...
    def add_transaction(self,s,r,v): 
        ...
        hashstring = str(ts) + str(s) + str(r) + str(v)
        tx_hash.update(hashstring.encode('utf-8'))#adds byte to hash
        tx_hash = tx_hash.hexdigest()#return digest as a string object of double length, containing only hexadecimal 
        #digits. This maybe used to exchange the value safely in email or other non-binary environments.
    ...
    def get_simple_merkle_root(self): 
        merkleString = self.__transactions["TxHash"].sum()
        
        merkleRoot = hashlib.sha256()
        merkleRoot.update(merkleString.encode('utf-8'))
        merkleRoot = merkleRoot.hexdigest()
        self.__merkle_tx_hash = merkleRoot
