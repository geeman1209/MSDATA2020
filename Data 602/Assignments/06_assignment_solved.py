...
def exercise09():
    ...
    # ------ Place code below here \/ \/ \/ ------
    df = requests.get('https://api.coindesk.com/v1/bpi/historical/close.json?start=2017-09-01&end=2018-10-05').json()
    df = pd.DataFrame(df)
    ...
    plotly_url = ply.line(df, x="Date", y="bpi", title='BitCoin Prices')
    plotly_url.show()
...
def exercise11(n):
    ...
    array_reshaped = array_1d.reshape(3, int((array_1d.shape[0])/3))
...
def exercise12(n):
    ...
    # ------ Place code below here \/ \/ \/ ------
    n = 2 * n
    checkerboard_matrix = np.ones((n, n))
    checkerboard_matrix[::2, 1::2] = 0
    checkerboard_matrix[1::2, ::2] = 0
    
    #np.tile([1,0], reps = (2 * n, 2 * n))
...
def exercise13(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    s = pd.Series(np.random.randn(n), index=pd.date_range('1/1/2010', periods=n, freq='D'))
...
#pd.Series(words)/df.apply(lambda x : len(x))
