...
def exercise01():
    ...
    # ------ Place code below here \/ \/ \/ ------
    iris2 = pd.DataFrame(ds.load_iris().data)   
    df_first_five_rows = iris2.head(5)
...
def exercise03(neighbors,split):
    ...
    X_train, X_test, y_train, y_test = tts(iris['data'], iris['target'], test_size = split, random_state = random_state, stratify = iris['target'])
...
def exercise04():
    ...
    X_train, X_test, y_train, y_test = tts(iris['data'], iris['target'], test_size = 0.3, random_state = 21, stratify = iris['target'])
...
def exercise05():
    ...
    # ------ Place code below here \/ \/ \/ ------
    boston_df = pd.DataFrame(ds.load_boston().data)
    boston_df.columns = ds.load_boston().feature_names    
    boston_values = pd.DataFrame(ds.load_boston().target)
    
    average_age = round(boston_df['AGE'].mean(), 2)
    average_medv = round(boston_values.mean(), 2)
    medv_as_numpy_values = np.array(boston_values)
    # ------ Place code above here /\ /\ /\ ------
...
def exercise06():
    ...
    plt.scatter(boston_df['PTRATIO'], boston_values)
...
def exercise07():
    ...
    y = np.array(boston_values).reshape(-1, 1)
    X_ptratio = np.array(boston_df['PTRATIO']).reshape(-1, 1)
