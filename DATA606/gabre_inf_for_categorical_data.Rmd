...
1.  In the first paragraph, several key findings are reported. Do these 
    ...
    Answer: The percentages appear to be sample statistics, since the results are from polling, which is considering a generalization of the population. 
...
Answer: It should be assumed that the sample is random and taken from less than 10% of the global population, which is a reasonable assumption.
...
Answer: The rows in table 6 represents a given country with their sample size, and percent values on how people responded. Atheism lists every individual response, their nationality, and the year the response was given.
...
Answer: Yes, it does agree, it is about 5%
```{r us-atheism, eval=TRUE}
us12 <- subset(atheism, nationality == "United States" & year == "2012")

length(us12$response[us12$response == 'atheist'])/length(us12$response[us12$response == 'non-atheist'])
```
...
Answer: The sampe size must be greater than 10, the sample size in this case is 1002. The observations must also be independent. I'm assuming independence but the condition of using a sample smaller than 10% of the population is also being met. 
...
Answer(a):    
```{r}
ir12 <- subset(atheism, nationality == "Ireland" & year == "2012")

inference(ir12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")

```
Margin of Error for Ireland = 1.8424%

There are 100 successes and 910 failures, the sample size is above 10 and below 10% of the general population. The population is randomly selected and therefor indepdent. 

Answer(b):
ME = 1.0192%

There are 33 successes, 1059 failures.

Like Ireland above, the sample size is above 10 and definitely below 10% of the general population. Sample is randomly selected and assumed to be independent. 
```{r}
...
Answer: Margin of error reaches the apex around 50% and decreases as p moves away from 50%.
...
Answer: The standard deviation is .00928. The minimum is 0.07019 and maximum is 0.12981. The distribution looks close to be normal.
```{r}
...
hist(p_hats, main = "p = 0.2, n = 1040", xlim = c(0, 0.36))
...
hist(p_hats, main = "p = 0.2, n = 400", xlim = c(0, 0.36))
...
Ecuador fails to meet the n > 10 condition for inference while Australia does meet the conditions. 
* * *
## On your own
...
Ho: The proportion of atheists in Spain has not changed between 2005 and 2012.

Ha: The proportion of Atheists in Spain has changed between 2005 and 2012.
...
Answer: There is no overlap and the null hypothesis can be rejected in favor of the alternative. 
